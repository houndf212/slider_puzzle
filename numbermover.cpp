...
std::pair<MoveVector, bool> NumberMover::find_null_to(Pos to, Board *board, const BoolMatrix &fixed)
{
    Pos null_start = board->get_null_pos();
    auto null_line = find_move_line(null_start, to, fixed);
    if (null_line.second == false)
        return std::make_pair(MoveVector(), false);
...
std::pair<PosVector, bool> NumberMover::find_move_line(Pos start, Pos finish, const BoolMatrix &fixed_matrix)
{
    if (start == finish)
        return std::make_pair(PosVector(), true);
